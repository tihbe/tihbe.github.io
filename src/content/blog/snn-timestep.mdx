...
<figure>
  <img src="/figures/snn-fig1.png" alt="Output firing rate as a function of timestep size for LIF neurons" />
  <figcaption>Output firing rate vs. $\delta t$ — the spiking frequency drops near $\delta t = 1$ ms.</figcaption>
</figure>
...
for refractory_period in [0.001, 0.01, 0.1]:
    ...
    ax.errorbar(dts * 1000, spike_rates.mean(axis=1), yerr=spike_rates.std(axis=1),
                capsize=5, label=f"Refrac.: {1000 * refractory_period:.1f}ms")
...
<figure>
  <img src="/figures/snn-fig2.png" alt="Output firing rate with explicit refractory periods" />
  <figcaption>Adding explicit refractory periods produces the same degradation pattern as large $\delta t$.</figcaption>
</figure>
...
<figure>
  <img src="/figures/snn-fig4.png" alt="Output firing rate with soft reset and multiple spikes per timestep" />
  <figcaption>With soft reset and multiple spikes per timestep, the firing rate is stable across all $\delta t$ values.</figcaption>
</figure>
